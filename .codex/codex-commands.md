...
> 本文件定义了 iLink 流水线中 4 个 AI 角色的命令指令。
> 当用户在对话中输入 `ilink-pm`、`ilink-design`、`ilink-coder`、`ilink-qa` 加 Story ID 时，按对应角色执行任务。
...
---
```
> 存在 H 级风险或逻辑矛盾时，Status 设为 `STAGING`
...
- STAGING → 展示 [UPSTREAM_BLOCKERS]，建议用户与 Designer 讨论
...
**Codex CLI 用户**：AI 读取 `.codex/codex-commands.md` 中对应角色章节执行任务。Shell 脚本用于准备输入和 Metadata 注入。
...
| `bash .codex/commands/ilink-inject-metadata <story> <role>` | 注入时间戳和哈希 |
...
ilink-init → 需求定义
    |
ilink-pm → pm.master.md (PENDING_DESIGNER / STAGING)
    |
ilink-design → design.master.md (STAGING) → [Human-Gate] → ilink-approve → PENDING_CODER
    |
ilink-coder → code.master.md + source files (PENDING_QA)
    |
ilink-qa → review.master.md
    ├─ COMPLETED → Done
    ├─ FAIL_BACK_TO_CODER → Reflux to ilink-coder
    └─ STAGING → Human intervention needed
```
