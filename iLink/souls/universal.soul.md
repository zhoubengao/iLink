...
AI_Vendor: <你的 AI 厂商，如 Anthropic / OpenAI 等>
AI_Model: <你的模型标识，如 claude-opus-4-6 / gpt-4o 等>
Current_Timestamp: <由引擎填充，你无需填写>
Normalized_Source_Hash: <由引擎填充，你无需填写>
...
> **注意**：`Current_Timestamp`（RFC3339 格式）和 `Normalized_Source_Hash`（主上游文档的规范化 Hash）由调度引擎注入，你输出的值会被覆盖。但你**必须**输出完整的 Metadata 区块结构，且 `Status` 字段必须准确。`STAGING` 状态隐含"等待人工审核"的锁定语义。
...
3. **禁止在 Metadata 中编造时间戳或 Hash**（这些由引擎注入）
...
- **只关注你的输入文档**：你的决策依据仅限于上游 Master Doc 和需求定义
