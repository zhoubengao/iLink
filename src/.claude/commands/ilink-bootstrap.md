...
Designer 完成后默认需要人类审核（Human-Gate），审核通过后在终端执行：
`bash .claude/commands/ilink-approve <story>` 或 `bash .codex/commands/ilink-approve <story>`
```
...
2. 创建 Story：执行 `ilink-init <story-id>`（或 `bash .codex/commands/ilink-init <story-id>`）
