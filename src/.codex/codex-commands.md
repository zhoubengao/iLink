...
- 如果有，读取该 Domain Knowledge 文件（重点参考 §2 业务实体、§6 设计决策、§9 见贤思齐），帮助做出与现有架构一致的技术设计
...
4. **完成见贤思齐**：选取国际同类产品或知名开源项目作为对标基准，独立完成赏析、风险识别、改进方向；品评基于模型知识，标注此局限。§9 MUST 包含两个子节：**见贤思齐 · 综评**（维度评级矩阵 + 综合评级 + 根本性反思）和 **见贤思齐 · 笃行**（紧急修复 / 能力补齐 / 架构演进三阶段 + 理念级提升），详见 domain.soul §4.3
5. **组装完整文档**：按十章标准格式输出

### 输出
- 写入 `iLink-doc/domain/<module>-domain-knowledge.md`
- 十章标准格式：§1 业务定位、§2 业务实体、§3 流程全景、§4 内部机制、§5 业务规则、§6 设计决策、§7 配置参数、§8 故障模式、§9 见贤思齐（含"综评 + 笃行"两个子节）、§10 待确认
...
| **C — 非 iLink** | 不含 iLink 关键词，但有项目说明 | 执行迁移流程 |

> **说明**：薄路由层不感知版本号，版本号由 `iLink/` 目录中的文件决定。用户升级 iLink 时只需替换 `iLink/` 目录，薄路由层无需改动。

**情况 A 操作**：直接创建 iLink 薄路由内容（见下文模板）。这样后续如果有人执行 Claude Code `/init`，会读到 iLink 引导而不会重新生成项目说明。
...
无论是新建（情况 A）还是迁移重写（情况 C），最终 `CLAUDE.md` MUST 包含以下完整内容（**保持精简，作为薄路由**）：
...
**情况 A 操作**：直接创建 iLink 薄路由内容（见下文模板）。
...
**如果发生 CLAUDE.md 或 AGENTS.md 迁移**（情况 C），必须额外输出以下警告：
...
| `bash .codex/commands/ilink-approve <story>` | 审核通过，推进 STAGING 状态 |
