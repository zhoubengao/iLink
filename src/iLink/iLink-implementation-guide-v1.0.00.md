...
系统不自建 LLM 调用层、不自建代码搜索引擎、不自建 Prompt 拼接模块，而是将这些能力完全交给 Host CLI。自研部分收敛为：**Soul 文件**（角色行为规范）+ **Slash Command**（执行编排）+ **轻量 bash 辅助脚本**（状态流转与 Metadata 注入）。整套系统的核心资产全部是纯 Markdown 文件，模型无关、平台可移植。
...
/ilink-design（技术设计 → 输出设计方案 + 文件级任务清单）
    ↓
⛔ Human-Gate：人类审核设计 → ilink-approve 推进
    ↓
...
| `inject_metadata()` | 注入 RFC3339 Timestamp 和 Normalized_Source_Hash（其他字段保留 Agent 填写，参见 Root Spec §5.4） |
...
| `strip_metadata_and_collapse_blanks()` | 剔除 Metadata 区块和连续空白行（用于 Hash 计算） |
...
| `ilink-approve <story>` | 将 STAGING 推进到下一个 PENDING 状态 |
| `ilink-inject-metadata <story> <role>` | 注入 RFC3339 Timestamp 和 Normalized_Source_Hash |
...
**步骤 2 — Command 文件检查**：
- 校验当前平台的 Command 文件（如 `.claude/commands/ilink-{pm,design,coder,qa,init}.md`）
...
- **其他 CLI 用户**：先读取 `iLink/iLink-root-spec-v1.0.00.md` §4，再读对应 Soul 文件执行
...
| 至少一个平台的 Command 文件集 | MUST | 如 `.claude/commands/ilink-{pm,design,coder,qa}.md` |
