...
2. [架构](#2-架构)
3. [流水线协议](#3-流水线协议)
4. [角色行为规范](#4-角色行为规范)（含 4.5 Domain Engineer）
...
| Domain Knowledge | 针对特定业务模块的领域认知文档，记录业务实体、流程全景、设计决策、实现品评等，存放于 `iLink-doc/domain/` |
| 认知资产 | 团队通过 Domain Knowledge 活动沉淀的可复用领域理解，独立于具体工单存在 |
| 实现品评 | Domain Knowledge 中对标国际同类产品和知名开源项目，对当前实现进行赏析与判断的章节 |
| [待确认] | Domain Knowledge 中 AI 无法从代码推导、需要业务专家补充的标记项 |
...
│   /ilink-pm → /ilink-design      ↓                          │
│   → ilink-approve                iLink-doc/domain/           │
│   → /ilink-coder → /ilink-qa     （领域认知资产库）            │
└────────────────────────┬─────────────────────────────────────┘
                         │ 手动触发
┌────────────────────────▼─────────────────────────────────────┐
...
6. **认知与交付分离**：Domain Knowledge 活动（认知线）与 Story 开发流程（交付线）MUST 保持独立，SHALL NOT 将 `ilink-domain` 纳入 PM→QA 流水线序列。Domain Knowledge 是交付线的**输入参考**，而非前置节点。
...
| §9 实现品评 | 对标国际同类产品的赏析与判断 | AI独立完成 |
| §10 待确认 | 代码里看不出来的空白，需业务专家补充 | AI标记 |

### 4.5.6 实现品评规范（§9）

实现品评是 Domain Knowledge 区别于普通技术文档的核心章节，MUST 遵守以下规范：
...
- Designer 角色：SHOULD 读取关联的 Domain Knowledge（重点参考 §2 业务实体、§6 设计决策、§9 实现品评），帮助做出与现有架构一致的技术设计
...
|------|-----------|----------------|
| PM | `<story>-需求定义.md` | `shasum iLink-doc/<story>/<story>-需求定义.md` |
...
| **修改上游重跑** | 根因在上游文档表述不清 | 修改上游文档（如需求定义.md），重新执行对应角色命令全量重生成 |
