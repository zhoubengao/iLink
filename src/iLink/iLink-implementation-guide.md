...
> **状态**: 正式发布
>
> **配套文档**：本文档是 `iLink-root-spec.md` 的配套实施手册。核心协议规范请参阅 Root Spec。
>
...
iLink 是一套 **CLI-native 的 AI 多角色软件开发协作系统**，可运行在 macOS、Windows、Linux 等主流操作系统上。它利用成熟的 Host CLI 工具（Claude CLI / Qoder CLI / Codex CLI / Gemini CLI）的原生 LLM 能力，以**文件状态机**驱动两条工作线：

**交付模式（v1.x）**：通过 Command 层编排 4 个专职 AI Agent（PM、Designer、Coder、QA），端到端完成软件交付流水线——从需求分析到技术设计、代码编写再到质量审查，关键节点人类审核，其余由 AI 完成。

**认知模式（v1.3+）**：通过 Domain Engineer 角色，帮助团队对重要的既有模块形成系统性的领域知识（Domain Knowledge）——AI 读取源码，提炼业务实体、流程全景、设计决策、见贤思齐，资深人员引导生成，沉淀为团队认知资产。

系统不自建 LLM 调用层、不自建代码搜索引擎、不自建 Prompt 拼接模块，而是将这些能力完全交给 Host CLI。自研部分收敛为：**Soul 文件**（角色行为规范）+ **Command 层**（执行编排，各平台以 Slash Command、bash 或混合方式落地）+ **轻量 bash 辅助脚本**（状态流转与流水线编排）。整套系统的核心资产全部是纯 Markdown 文件，模型无关、平台可移植。
...
- **可追溯的决策链**：每个 Story 完成后留下完整的文档链——`<id>-requirement.md` → `pm.master.md` → `design.master.md` → `code.master.md` → `review.master.md`。每份文档末尾的 Metadata 印章记录了角色、AI 模型、时间戳、Upstream_SHA1，构成天然的审计材料，适配金融、政务、医疗等合规敏感领域。
...
/ilink-pm（理解需求 → 输出业务合同）
    ↓
├─ STAGING → /ilink-refine（修订文档，仍保持 STAGING）→ /ilink-approve 正式推进
└─ PENDING_DESIGNER →
...
每一步由人类在 CLI 中手动触发对应的角色命令，AI Agent 在 Host CLI 的对话上下文中执行任务，产出结构化的 Master Doc 并写入磁盘。整个流水线的状态完全由文件承载——Master Doc 末尾的 Metadata 印章（Status + Upstream_SHA1）+ 信号文件（`.retry_count`），不依赖任何内存状态。

> **注意**：`/ilink-refine` 是修订对话，仅更新文档内容，完成后文档仍保持 STAGING 状态；`/ilink-approve` 才是正式的状态推进动作，且仅接受当前 Status=STAGING 的文档。

#### 认知工作线（资深引导）

认知工作线**独立于交付流水线**，不绑定工单，由资深工程师判断哪些模块值得沉淀。包含两类活动：

**1. Domain Knowledge（业务领域沉淀）**

```
资深工程师选定模块 → /ilink-domain <module-id>
    ↓
AI 读取源码 → 提炼业务实体、流程全景、线程模型、设计决策
    ↓
AI 生成 10 章标准领域知识文档（domain-knowledge.md）
    ↓
资深工程师审核 → 确认 §10 [待确认] 项 → 文档定稿
    ↓
git commit → 沉淀为团队认知资产（iLink-doc/domain/）
```

**2. SDD Assessment（SDD 适配度评估，v1.4.11 新增）**

```
资深工程师/架构师 → /ilink-sdd [scope]
    ↓
AI 读取代码与架构事实 → 应用三步过滤链 + 实例规模标尺 + 内部 DSL 覆盖度判据
    ↓
AI 生成 9 章标准评估报告（sdd-assessment-<scope>.md）
    ↓
资深工程师审核 → 确认评估结论 → 文档定稿
    ↓
git commit → 沉淀为元决策依据（iLink-doc/sdd/）
```

两类认知活动的差异：

| 维度 | Domain Knowledge | SDD Assessment |
|------|-----------------|----------------|
| **沉淀对象** | 业务领域理解 | 模块是否适合 SDD 的元决策 |
| **触发场景** | 模块对未来 Story 有持续价值 | 团队推进 SDD 实践或决策需求 |
| **服务对象** | 后续 PM / Designer 的领域参考 | "是否引入 SDD"的元决策 |
| **资产路径** | `iLink-doc/domain/` | `iLink-doc/sdd/` |

与交付模式的核心区别：

| 维度 | 交付模式 | 认知模式 |
|------|---------|---------|
| **触发方** | 所有开发者（工单驱动） | 资深工程师/架构师（主动判断） |
| **输入** | requirement.md（人类新写） | 既有源码 + 架构事实（AI 读取） |
| **输出** | 可运行的代码 + 文档链 | 领域知识 / SDD 评估报告（认知资产） |
| **频率** | 每个工单一次 | 按需，重要模块或方法论决策时 |
| **价值归宿** | 系统功能 | 团队能力 + 元决策能力 |
...
| **认知与交付分离** | 领域知识沉淀（认知模式）与工单开发（交付模式）是独立工作线，不互相阻塞，各有其触发条件和产出形式 |
...
- Gemini CLI：使用 `.gemini/commands/*.toml` TOML 格式配置文件，通过 `{{args}}` 占位符接收用户参数
...
| `ilink-approve <story>` | 将 STAGING 推进到下一个状态 |

#### 需求定义模板

`ilink-init` 生成的需求定义模板包含以下标准章节：
...
<!-- 如果本需求涉及已有 Domain Knowledge 的模块，在此指定文件路径 -->
<!-- PM 和 Designer 将据此读取对应的领域知识作为参考 -->
iLink-doc/domain/login-410301-domain-knowledge.md
```

> **关联领域知识**字段（v1.3 新增）是可选的。人类在编写需求时，如果判断本次需求涉及已有 Domain Knowledge 的模块，可在此指定文件路径。PM 和 Designer 会据此读取对应的 Domain Knowledge 作为辅助参考。无关联时删除本节或留空。
...
**步骤 1 — 框架文件检查**：
- 校验 `iLink/iLink-root-spec.md` 与 `iLink/souls/{universal,pm,design,coder,qa,domain,sdd}.soul.md` 是否存在
- 全部缺失时 MUST 停止执行，提示用户先复制 iLink 框架到项目

**步骤 2 — Command 文件检查**：
- 校验当前平台的 Command 文件（如 `.claude/commands/ilink-{pm,design,coder,qa,init,refine,approve,status,domain,sdd}.md`）
...
| **B — 已是 iLink** | 包含 `iLink 协作协议` 关键词 | 跳过，无需修改 |
| **C — 非 iLink** | 不含 iLink 关键词，但有项目说明 | 执行**迁移流程**（见下） |

**情况 C 的迁移流程**（MUST 严格遵守）：
...
处理逻辑**与步骤 5 完全一致**——按 A/B/C 三种情况判断，情况 C 执行同样的迁移流程：

| 情况 | 判定条件 | 处理动作 |
|------|---------|---------|
| **A — 不存在** | 文件缺失 | 创建包含 iLink 薄路由的最小文件 |
| **B — 已是 iLink** | 包含 `iLink 协作协议` 关键词 | 跳过，无需修改 |
| **C — 非 iLink** | 不含 iLink 关键词，但有项目说明 | 执行迁移流程：备份为 `AGENTS.md.bak.<YYYYMMDD_HHMMSS>` → 确认内容已迁移到 project-context.md → 重写为薄路由 → 告知用户 |
...
| Gemini CLI | — | 通过 `.gemini/commands/` 目录自动发现 |
...
- MUST 指向 `iLink/iLink-root-spec.md` 和 `project-context.md`
...
本项目使用 iLink 多角色 AI 协作流水线（Root Spec v1.5.0）。
...
/ilink-approve <story>  → Human-Gate 推进：STAGING → 下一状态
/ilink-status [story]   → 查看当前流水线状态与下一步建议
/ilink-domain <module>  → 认知模式：AI 读源码 → 生成领域知识文档（资深人员触发）
/ilink-sdd [scope]      → 认知模式：AI 评估 SDD 适配度 → 生成评估报告（资深人员/架构师触发）
```
...
| 文件 | 必要性 | 说明 |
|------|-------|------|
| `iLink/iLink-root-spec.md` | MUST | 根规范 |
| `iLink/iLink-implementation-guide.md` | SHOULD | 实施手册 |
...
| `iLink/souls/sdd.soul.md` | SHOULD | SDD Assessment Engineer 角色规范（认知模式，v1.4.11 新增） |
...
| `iLink-doc/sdd/` 目录 | SHOULD（目录本身——不是每个项目都做 SDD 评估） | SDD 适配度评估报告存放目录（一旦产出了文档，MUST 纳入版本控制） |
