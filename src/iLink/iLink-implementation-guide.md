...
认知工作线**独立于交付流水线**，不绑定工单，由资深工程师判断哪些模块值得沉淀。
...
| 维度 | 交付模式 | 认知模式 |
|------|---------|---------|
| **触发方** | 所有开发者（工单驱动） | 资深工程师（主动判断） |
| **输入** | 需求定义.md（人类新写） | 既有源码（AI 读取） |
| **输出** | 可运行的代码 + 文档链 | 领域知识文档（认知资产） |
| **频率** | 每个工单一次 | 按需，重要模块才做 |
| **价值归宿** | 系统功能 | 团队能力 |

领域知识文档存放在 `iLink-doc/domain/`，MUST 纳入版本控制。
...
**步骤 1 — 框架文件检查**：
- 校验 `iLink/iLink-root-spec.md` 与 `iLink/souls/{universal,pm,design,coder,qa,domain}.soul.md` 是否存在
- 全部缺失时 MUST 停止执行，提示用户先复制 iLink 框架到项目

**步骤 2 — Command 文件检查**：
- 校验当前平台的 Command 文件（如 `.claude/commands/ilink-{pm,design,coder,qa,init,refine,approve,status,domain}.md`）
...
本项目使用 iLink 多角色 AI 协作流水线（Root Spec v1.4.00）。
