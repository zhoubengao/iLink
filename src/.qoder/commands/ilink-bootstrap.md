...
**情况 B：CLAUDE.md 已是 iLink 引导**（包含 `iLink 协作协议` 关键词）
→ 跳过，无需修改。薄路由层不感知版本号，版本号由 `iLink/` 目录中的文件决定。

**情况 C：CLAUDE.md 是其他 AI 工具生成的项目说明**（不含 iLink 协作协议关键词）⭐
→ 执行**迁移流程**：
...
**情况 B：AGENTS.md 已是 iLink 引导**（包含 `iLink 协作协议` 关键词）
→ 跳过，无需修改。薄路由层不感知版本号，版本号由 `iLink/` 目录中的文件决定。

**情况 C：AGENTS.md 是其他 AI 工具（Qoder/Codex `/init` 等）生成的项目说明** ⭐
→ 执行**迁移流程**：
...
无论是新建（情况 A）还是迁移重写（情况 C），确保最终 `AGENTS.md` 包含以下核心内容（**保持精简，作为薄路由**）：
